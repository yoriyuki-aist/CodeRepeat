		       ...
		       SArrayIterator sa, SArrayIterator sa_end)
{
  ...
  if (stringsort_only || very_short_string) {
    ...
    TIME_CHECK(intermediate); // Step 1pre
    TIME_CHECK(intermediate); // Step 1
    TIME_CHECK(intermediate); // Step 2pre
    TIME_CHECK(intermediate); // Step 2
...
#ifdef DEBUG
    std::cerr << "Sorting only to depth " << period-1 << std::endl;
#endif

    suffix_mkqsort_noempty(str, str_end, sa, sa_end, 
			   period-1, do_nothing());

    TIME_CHECK(intermediate); // Step 3
    TIME_CHECK(intermediate); // Step 4
...
#ifdef DEBUG
    std::cerr << "Starting sample sort" << std::endl;
#endif
...
#ifdef DEBUG
    std::cerr << "Initializing for doubling" << std::endl;
#endif
...
#ifdef DEBUG
    std::cerr << "Starting doubling" << std::endl;
#endif
...
#ifdef DEBUG
      std::cerr << "Starting full suffix array sort" << std::endl;
#endif
...
#ifdef DEBUG
      std::cerr << "Starting final sort" << std::endl;
#endif
